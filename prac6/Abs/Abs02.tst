load Abs.vm,
output-file Abs02.out,
compare-to Abs02.cmp,
output-list sp%D1.6.1 local%D1.6.1 argument%D1.8.1 this%D1.6.1 that%D1.6.1
            RAM[16]%D1.6.1 RAM[17]%D1.6.1 RAM[18]%D1.6.1
            local[0]%D1.8.1 local[1]%D1.8.1 local[2]%D1.8.1
            argument[0]%D1.11.1 argument[1]%D1.11.1 argument[2]%D1.11.1;

set sp 256,        // stack pointer
set local 512,     // base address of the local segment (larger)
set argument 100,  // base address of the argument segment (smaller)
set this 4000,     // base address of the this segment
set that 4010,     // base address of the that segment

set RAM[16] -42,   // static 0
set RAM[17] -42,  // static 1
set RAM[18] 7,    // static 2

set local[0] -1,    // local 0
set local[1] -2,    // local 1
set local[2] -3,    // local 2

set argument[0] 123,   // argument 0
set argument[1] 0,     // argument 1
set argument[2] -999;  // argument 2

repeat 300 {       
  vmstep;
}
output;
